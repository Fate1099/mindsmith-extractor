## README.md
    ...
    The generated Markdown is designed for pandoc + pdflatex (no xelatex needed):

        pandoc "LE 1 - Example.md" -o "LE 1 - Example.pdf"

    The YAML front matter provides the title and subtitle automatically.
    German umlauts and special characters are fully supported under
    pdflatex + Latin Modern.
    ...
    ### v1.5
    - Fix: heading-only list items no longer produce orphan bullet points
    - Fix: quiz feedback rendered inline with em-dash instead of broken
      nested blockquote
    - Fix: sectionCalloutTile content now captured (was silently dropped)
    - Fix: consecutive thematic breaks collapsed into one
    - Fix: page markers are HTML comments (no inverted heading hierarchy)
    - Fix: stray space before colon after emphasis cleaned up
    - Fix: intentionally-skipped tiles (buttonsTile, spacerTile) emit no
      comment noise
    - Add: LaTeX RaggedRight on section headings via header-includes
      (prevents long heading overflow in pdflatex)
    - Replace all emojis with ASCII labels for pdflatex compatibility
    - Single-block table assembly (no blank-line row shredding)
    - Pipe escaping in table cells
    - Pandoc-native underline spans instead of raw HTML
    - Tight lists as single strings
    - Thematic breaks use asterisks instead of dashes
    - YAML front matter with title/subtitle
    - Pandoc hard breaks preserved outside tables
    - WebP/SVG image format guard
    - Context-aware rendering (inTable flattens content)

    ### v1.2
    - Self-contained download (no background script)
    - Works inside LMS iframes (all_frames, document_start)
    - Page-world fetch/XHR interception via inject.js
